...
export DISTDIR=${DISTDIR:-"${EROOT}/var/cache/distfiles"}
...
export LAYMAN_BASEDIR=${LAYMAN_BASEDIR:-"${EROOT}/var/lib/layman"}
...
export OILEDMACHINE_OVERLAY_DIR=${OILEDMACHINE_OVERLAY_DIR:-"${EROOT}/usr/local/oiledmachine-overlay"}
export OPENGL_OPT="O3.conf"
export PORTAGE_DIR=${PORTAGE_DIR:-"${EROOT}/usr/portage"} # with ebuilds
export PORTAGE_ETC=${PORTAGE_DIR:-"${EROOT}/etc/portage"} # with package.accept_keywords ; can be ${DIR_SCRIPT}
export T=$(mktemp -d)
...
export VAR_PATH=${VAR_PATH:-"${EROOT}/var/cache/gen_crypto_package"} # can be $(realpath $(pwd))
export WOPT=${WOPT:-"20"}
export WPKG=${WPKG:-"50"}
