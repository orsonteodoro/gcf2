...
main() {
	if [[ ! -e "${EROOT}/etc/portage/emerge-cfi-skip.lst" ]] ; then
		echo "[warn] Missing emerge-cfi-skip.lst.  Run gen_pkg_lists.sh"
	fi
	if [[ ! -e "${EROOT}/etc/portage/emerge-cfi-no-cfi.lst" ]] ; then
	...
	L_SKIP=($(\
		for x in $(cat \
			"${EROOT}/etc/portage/emerge-cfi-skip.lst" \
			"${EROOT}/etc/portage/emerge-cfi-no-cfi.lst" \
		) ; do \
			echo "${x/:}" ; \
		done \
	))
