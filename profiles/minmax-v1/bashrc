...
gcf_check_packages() {
	if [[ "${CATEGORY}/${PN}" == "dev-libs/libgcrypt" ]] \
		&& [[ "${CC}" =~ "clang" \
			&& "${CATEGORY}/${PN}" == "dev-libs/libgcrypt" ]] ; then
		gcf_warn "You must enable dev-libs/libgcrypt with O-flag-munging"
	fi
}
...
gcf_warn "Skipping the Souper optimization pass."
		...
		gcf_append_ldflags -Wl,--no-as-needed -Wl,-lLLVM-${s_llvm} -Wl,-rpath=/usr/lib/souper/${s_llvm}/$(get_libdir) -Wl,-rpath=/usr/lib/llvm/${s_llvm}/$(get_libdir)
		if [[ "${USE_SOUPER_SIZE}" ]] \
...
gcf_warn "may needed to be added to per-package *FLAGS to fix an external or"
