...
is_already_added_to_system_set() {
	for t in ${LTO_CATS[@]} ; do
		grep -q -E -e "${pp}$" "${EROOT}/etc/portage/emerge-system-${t}.lst" && return 0
	done
	return 1
}
...
check_static_libs() {
	...
	for c in ${LTO_CATS[@]} ; do
		echo "# version ${VERSION}" > "${EROOT}/etc/portage/emerge-${emerge_set}-${c}.lst"
	done
	local p
	for p in ${L[@]} ; do
		...
		local path2=$(realpath "${EROOT}/var/db/pkg/${pp}-${v_mm2}"*"/CONTENTS" 2>/dev/null | head -n 1)
		local path3=$(realpath "${EROOT}/var/db/pkg/${pp}-${v_mm1}"*"/CONTENTS" 2>/dev/null | head -n 1)
		local path4=$(realpath "${EROOT}/var/db/pkg/${pp}-"*"/CONTENTS" 2>/dev/null | head -n 1)
		...
		if [[ -e "${EROOT}/var/db/pkg/${p}/CONTENTS" ]] ; then
			for f in $(cat "${EROOT}/var/db/pkg/${p}/CONTENTS" | cut -f 2 -d " ") ; do
				check_ltoable
			done
			...
			echo "[no-data] ${p}"
			echo "${pp}" >> "${EROOT}/etc/portage/emerge-${emerge_set}-no-data.lst"
			continue
		fi
		if (( ${has_static} == 0 && ${has_shared} == 0 && ${has_exe} == 0 )) ; then
			...
			echo "[lto-skip] ${p}"
			echo "${pp}" >> "${EROOT}/etc/portage/emerge-${emerge_set}-lto-skip.lst"
		elif (( ${has_static} == 0 )) ; then
			TOTAL_LTO_PACKAGES=$(( ${TOTAL_LTO_PACKAGES} + 1 ))
			N_LTO_AGNOSTIC=$(( ${N_LTO_AGNOSTIC} + 1 ))
			echo "[lto-agnostic] ${p}"
			echo "${pp}" >> "${EROOT}/etc/portage/emerge-${emerge_set}-lto-agnostic.lst"
		else
			TOTAL_LTO_PACKAGES=$(( ${TOTAL_LTO_PACKAGES} + 1 ))
			if [[ "${emerge_set}" == "system" && "${CC_LTO}" != "${CC_LIBC}" ]] ; then
				N_NO_LTO=$(( ${N_NO_LTO} + 1 ))
				echo "[no-lto] ${p}"
				echo "${pp}" >> "${EROOT}/etc/portage/emerge-${emerge_set}-no-lto.lst"
			elif [[ "${emerge_set}" == "system" && "${CC_LTO}" == "${CC_LIBC}" ]] ; then
				N_LTO_RESTRICTED=$(( ${N_LTO_RESTRICTED} + 1 ))
				echo "[lto-restricted] ${p}"
				echo "${pp}" >> "${EROOT}/etc/portage/emerge-${emerge_set}-no-restricted.lst"
			else
				N_LTO_RESTRICTED=$(( ${N_LTO_RESTRICTED} + 1 ))
				echo "[lto-restricted] ${p}"
				echo "${pp}" >> "${EROOT}/etc/portage/emerge-${emerge_set}-lto-restricted.lst"
			fi
		fi
	done
...
echo "Legend:"
echo
echo "no-data - Data is stored in ${EROOT}/etc/portage/emerge-${emerge_set}-no-data.lst."
echo "     Requires emerge to generate a installed files list for the package"
echo "     or manual placement."
echo
echo "no-lto - Data is stored in ${EROOT}/etc/portage/emerge-${emerge_set}-no-lto.lst."
echo "     LTO must be disabled unless LTO compiler matches the same one used"
echo "     by libc."
echo
echo "lto-agnostic - Data is stored in ${EROOT}/etc/portage/emerge-${emerge_set}-lto-agnostic.lst."
echo "     No IR incompatibilities with binaries in the package.  Any compiler will work."
echo
echo "lto-skip - Data is stored in ${EROOT}/etc/portage/emerge-${emerge_set}-lto-skip.lst."
echo "     No LTO is required."
echo
echo "lto-restricted - Data is stored in ${EROOT}/etc/portage/emerge-${emerge_set}-lto-restricted.lst."
...
is_ltoable() {
	if grep -q -E -e "^${pp}$" "${EROOT}/etc/portage/emerge-{system,world}-{lto-agnostic,lto-restricted}.lst" ; then
		return 0
	fi
	return 1
}
...
is_lto_restricted() {
	if grep -q -E -e "^${pp}$" "${EROOT}/etc/portage/emerge-{system,world}-lto-restricted.lst" ; then
		return 0
	fi
	return 1
}
...
is_cfi_entry_added() {
	...
	for c in ${CFI_CATS[@]} ; do
		if grep -q -e "^${pp}(:|$)" "${EROOT}/etc/portage/emerge-${c}.lst" ; then
			return 0
		fi
	done
	return 1
...
is_world_set() {
	local c
	for c in ${LTO_CATS[@]} ; do
		if grep -q -e "^${pp}$" "${EROOT}/etc/portage/emerge-world-${c}.lst" ; then
			return 0
		fi
	done
	return 1
...
is_system_set() {
	local c
	for c in ${LTO_CATS[@]} ; do
		if grep -q -e "^${pp}$" "${EROOT}/etc/portage/emerge-system-${c}.lst" ; then
			return 0
		fi
	done
	return 1
...
check_cfi() {
	...
	echo
	for c in ${CFI_CATS[@]} ; do
		echo "# version ${VERSION}" > "${EROOT}/etc/portage/emerge-${c}.lst"
	done
	...
	for p in ${L[@]} ; do
		...
		local nbin=0
		if [[ -e "${EROOT}/var/db/pkg/${p}/CONTENTS" ]] ; then
			set_cfi_flags "${EROOT}/var/db/pkg/${p}/CONTENTS"
		elif [[ -e "${path2}" ]] 2>/dev/null 1>/dev/null ; then
			# Fallback template:  ${PN}-1.2*
			echo "[warn] No ${p} installation found.  Using older ${pp} ebuild for inspection"
			set_cfi_flags "${path2}"
			...
			echo "[no-data] ${p}:"
			echo "${pp}" >> "${EROOT}/etc/portage/emerge-cfi-no-data.lst"
			continue
		fi
...
echo "Legend:"
echo
echo "no-data - Data is stored in ${EROOT}/etc/portage/emerge-cfi-no-data.lst."
echo "    It requires emerge to generate a installed files list for the"
echo "    package or manual placement."
echo
echo "no-cfi - Data is stored in ${EROOT}/etc/portage/emerge-cfi-no-cfi.lst.  Not"
echo "    allowed to be CFIed."
echo
echo "cfi-skip - Data is stored in ${EROOT}/etc/portage/emerge-cfi-skip.lst.  Doesn't"
echo "    require being CFIed."
echo
echo "cfi-world - Data is stored in ${EROOT}/etc/portage/emerge-cfi-world.lst."
echo "    Only allowed when CC_LTO=clang otherwise disabled for that set."
echo
echo "cfi-system - Data is stored in ${EROOT}/etc/portage/emerge-cfi-system.lst."
...
main() {
	export CC_LTO=$(grep -F -e "CC_LTO" "${EROOT}/etc/portage/make.conf" | cut -f 2 -d "=" | sed -e "s|#.*||" -e 's|"||g')
	export CC_LIBC=$(grep -F -e "CC_LIBC" "${EROOT}/etc/portage/make.conf" | cut -f 2 -d "=" | sed -e "s|#.*||" -e 's|"||g')

	if [[ -z "${CC_LTO}" ]] ; then
		echo "Missing CC_LTO in ${EROOT}/etc/portage/make.conf.  Valid values clang, gcc."
		exit 1
	fi

	if [[ -z "${CC_LIBC}" ]] ; then
		echo "Missing CC_LIBC in ${EROOT}/etc/portage/make.conf.  Valid values clang, gcc."
		exit 1
	fi
