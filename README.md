...
* /etc/portage/emerge-system-lto-restricted.lst -- Packages in this list are only allowed the default compiler for @system.  If no static-libs are linked, can use LTO; otherwise, LTO must be disabled.
...
* /etc/portage/emerge-world-lto-restricted.lst -- Packages in this list are only allowed the default compiler without LTO for @world if statically-linked otherwise may use the preferred LTO compiler
