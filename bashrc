...
gcf_error
gcf_error "(1) Manually recategorize the package in /etc/portage/emerge*.lst if"
gcf_error "temporarly blocked."
gcf_error "(2) Try disabling all CFI flags first, and if it works then converge"
gcf_error "towards the minimal CFI exception set for this package."
gcf_error "(3) Disable CFI for this package.  UBSan may still need to be linked."
gcf_error "(4) Switch back to GCC."
gcf_error "(5) If this package is placed in the no-data LTO list, disable CFI"
...
gcf_error "(6) If this package is permenently blacklisted (because it contains"
...
gcf_verify_cfi() {
	...
	for f in $(find "${ED}" -name "*.so*") ; do
		...
		if (( ${is_so} == 1 )) ; then
			if objdump -T "${f}" 2>/dev/null | grep -E -q -e "(__cfi_init|__cfi_check_fail)" ; then
				:;
			else
