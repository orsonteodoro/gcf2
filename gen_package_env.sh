...
LOC_OVERLY_LONG_BUILD_TIMES_OPT="O1.conf" # Optimize for build speed instead.  Often it is the data size not the code that is the problem.
LOC_OVERLY_LONG_BUILD_TIMES_SIZE=${LOC_OVERLY_LONG_BUILD_TIMES_SIZE:-10000000} # 10M ELOC ; Match the number for 8 hr builds
...
gen_ssa_opt_list() {
	...
	for op in ${OVERLAY_PATHS[@]} ; do
		for path in $(find "${op}/" -name "Manifest") ; do
			...
			if (( ${loc} >= ${LOC_OVERLY_LONG_BUILD_TIMES_SIZE} )) ; then
				printf "%-${WPKG}s%-${WOPT}s %s\n" "${p}" "${LOC_OVERLY_LONG_BUILD_TIMES_OPT}" "# Archive size: ${filesize} ; Estimated MLOC: ${mloc}" >> package.env.t.loc
...
gen_loc_list() {
	echo "" >> package.env
	echo "# Reducing build times for large projects by half relative to -O3" >> package.env
	echo "# Autogenerated list" >> package.env
	echo "" >> package.env
